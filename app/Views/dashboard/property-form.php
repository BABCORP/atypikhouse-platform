...
<section class="section form-layout">
    <form class="panel" method="post" enctype="multipart/form-data" action="<?= $isEdit ? url('/proprietaire/logements/' . $property['id'] . '/modifier') : url('/proprietaire/logements/ajouter') ?>" data-track="property_submit">
        ...
        <label>Équipements, séparés par des virgules<input name="amenities" placeholder="Spa, Wifi, Petit-déjeuner"></label>
        <label>Image principale (JPG, PNG ou WEBP, 5 Mo max.)<input type="file" name="property_image" accept="image/jpeg,image/png,image/webp"></label>
